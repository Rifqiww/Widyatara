...
export default function KayuMaleleGame() {
  ...
  const [gameSubtitle, setGameSubtitle] = useState(
    "Tap layar untuk mulai melempar",
  );
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      spectatorsRef.current.forEach(
        (s, i) => (s.position.y = Math.sin(t + i) * 0.1),
      );
  ...
  const createSpectator = async () => {
    ...
    const modelPath =
      spectatorCount % 2 === 0 ? "/model/Cewe.glb" : "/model/Cowo.glb";
  ...
  const setupInput = () => {
    ...
    const handlePointerDown = (e: PointerEvent) => {
      ...
      if (stateRef.current === "START" || stateRef.current === "RESET") {
        ...
        if (indukStickRef.current) {
          (
            indukStickRef.current.material as THREE.MeshStandardMaterial
          ).color.setHex(0xc19a6b);
        }
      }
    };
    ...
    const handlePointerMove = (e: PointerEvent) => {
      ...
      if (indukStickRef.current) {
        indukStickRef.current.rotation.z = Math.max(
          Math.min(-Math.PI / 3 - deltaY, 0),
          -Math.PI,
        );
    ...
    const handlePointerUp = (e: PointerEvent) => {
      ...
      const inHitZone =
        stickHeight >= CONFIG.hitZoneHeight.min &&
        stickHeight <= CONFIG.hitZoneHeight.max;
  ...
  const performHit = (vector: THREE.Vector3) => {
    ...
    anakRotationVelocityRef.current.set(
      Math.random() * 0.8,
      Math.random() * 0.8,
      Math.random() * 0.8,
    );
  ...
  const resetIndukStick = () => {
    ...
    const back = () => {
      ...
      if (progress <= 1 && indukStickRef.current) {
        indukStickRef.current.rotation.z = THREE.MathUtils.lerp(
          startRotZ,
          -Math.PI / 3,
          progress,
        );
        indukStickRef.current.rotation.y = THREE.MathUtils.lerp(
          startRotY,
          0,
          progress,
        );
  ...
  const calculateScore = () => {
    ...
    setGameTitle("MANTAP!");
    setGameSubtitle(`Jarak: ${scoreValue.toFixed(1)} Meter`);
    ...
    <div className="fixed inset-0 w-screen h-screen overflow-hidden font-sans touch-none z-50">
      ...
      <div className="absolute top-4 left-4 z-30">
        <button
          ...
          className="cursor-pointer bg-primary hover:bg-primary/80 backdrop-blur-sm text-white px-4 py-2 rounded-lg flex items-center gap-2 transition-all shadow-xl border border-white/20"
        >
      ...
      {/* UI Layer */}
      <div className="absolute top-0 left-0 w-full h-full pointer-events-none flex flex-col justify-between items-end p-5 z-40">
        {/* Score Board - Right Side, Hidden on Mobile Start */}
        {showScoreboard && (
          <div className="bg-black/60 backdrop-blur-md text-white p-3 md:p-4 rounded-2xl text-sm md:text-lg max-w-[180px] md:max-w-xs shadow-2xl border border-white/20 select-none animate-in fade-in slide-in-from-right-4 duration-500">
            <strong className="text-yellow-400 block mb-1 md:mb-2 text-xs md:text-base uppercase tracking-wider">
              Kayu Malele
            </strong>
            <div className="space-y-1 md:space-y-2 mb-3">
              <div className="flex justify-between items-center gap-2">
                <span className="opacity-80 text-[10px] md:text-sm uppercase font-semibold">
                  Skor
                </span>
            ...
            <div className="bg-white/5 p-2 rounded-xl text-[9px] md:text-xs leading-relaxed border border-white/5">
              <p className="flex gap-1.5">
                <span>
                  <b>Klik/Tap</b> untuk lempar ke atas.
                </span>
              </p>
              <p className="flex gap-1.5 mt-1">
                <span>
                  <b>GESER CEPAT</b> untuk memukul!
                </span>
              </p>
            </div>
          </div>
        ...
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center text-white pointer-events-none">
          {gameTitle && (
            <h1 className="m-0 text-5xl text-yellow-400 drop-shadow-lg">
              {gameTitle}
            </h1>
          )}
          <p className="text-xl mt-2 drop-shadow-lg font-bold text-yellow-300">
            {gameSubtitle}
          </p>
          {showResetBtn && (
            <button
