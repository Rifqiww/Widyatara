...
class AudioManager {
  ...
  stopPourSound() {
    if (this.pourNode) {
      // Fade out volume
      this.pourNode.gain.linearRampToValueAtTime(
        0,
...
export default function PapedaGame() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (p.gameActive) {
        ...
        if (p.lumpiness >= 100)
          handleGameOver(
            "Gagal! Sagu menggumpal keras karena disiram air panas tanpa diaduk.",
          );
        else if (p.spillRisk >= 100)
          handleGameOver(
            "Gagal! Cipratan air panas membahayakan. Pelan sedikit!",
          );
        ...
        const wobble =
          1 + Math.sin(Date.now() * 0.015) * 0.03 * p.angularVelocity;
    ...
    const handleMouseMove = (e: MouseEvent) => {
      if (!paramsRef.current.gameActive || !paramsRef.current.isDragging)
        return;
  ...
  return (
    <main className="fixed inset-0 w-screen h-screen overflow-hidden bg-neutral-900 text-white font-sans selection:bg-amber-500 selection:text-black z-50">
      ...
      {/* GAME UI OVERLAY */}
      <div
        className={`absolute inset-0 z-10 pointer-events-none transition-opacity duration-300 ${gameState === "PLAYING" ? "opacity-100" : "opacity-0"}`}
      >
        <div className="flex flex-col h-full justify-between p-4 md:p-6">
          ...
          <div className="absolute top-4 left-4 z-20">
            <button
              ...
              className="cursor-pointer bg-primary hover:bg-primary/80 backdrop-blur-sm text-white px-4 py-2 rounded-lg flex items-center gap-2 transition-all shadow-xl border border-white/20 pointer-events-auto"
            >
          ...
          {/* HUD */}
          <div className="flex flex-col md:flex-row justify-between items-start gap-3 md:gap-6 max-w-5xl mx-auto w-full mt-16 md:mt-4">
            {/* Left: Progress */}
            ...
            <div className="flex-[1.5] w-full text-center hidden md:block">
              <h2
                className={`text-base md:text-2xl font-bold transition-colors duration-200 ${feedback.includes("Gagal") || feedback.includes("ADUK") ? "text-red-400 animate-pulse" : "text-amber-300"}`}
              >
                {feedback}
              </h2>
            </div>
            ...
            {isMobile ? (
              ...
              <div className="space-y-2">
                <button
                  onTouchStart={(e) => {
                    e.preventDefault();
                    if (
                      paramsRef.current.gameActive &&
                      !paramsRef.current.isPouring
                    ) {
                      paramsRef.current.isPouring = true;
                  ...
                  className="pointer-events-auto w-full max-w-sm mx-auto active:scale-95 transition-transform bg-linear-to-r from-blue-600 to-blue-500 active:from-blue-500 active:to-blue-600 text-white font-extrabold rounded-lg px-8 py-5 text-lg shadow-xl"
                >
                  💧 TAHAN UNTUK TUANG AIR
                </button>
                <p className="text-xs text-white/60">
                  Drag jari untuk mengaduk
                </p>
              </div>
            ) : (
              ...
              <div className="inline-flex gap-6 items-center bg-black/30 px-6 py-3 rounded-lg">
                <div className="flex items-center gap-2">
                  <kbd className="px-3 py-1 bg-white/90 text-black font-bold rounded text-xs">
                    SPACE
                  </kbd>
      ...
      {/* START SCREEN */}
      {gameState === "START" && (
        <div className="absolute inset-0 z-50 bg-black/95 flex flex-col items-center justify-start md:justify-center p-6 text-center overflow-y-auto scrollbar-hide pb-24">
          <h1 className="text-5xl md:text-7xl font-bold text-transparent bg-clip-text bg-linear-to-r from-amber-400 to-orange-600 mb-4 tracking-tighter drop-shadow-sm mt-10 md:mt-0">
            The Art of Papeda
          </h1>
          <p className="text-lg md:text-xl text-gray-400 italic mb-6 md:mb-10 max-w-2xl font-light">
            "Harmoni antara tangan {isMobile ? "kiri" : "kiri menuang"} dan
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-300 mt-2 leading-relaxed">
                  ...
                  Kamu harus menuang air{" "}
                  {isMobile ? "(tombol biru)" : "(Spasi)"} sambil terus mengaduk{" "}
                  {isMobile ? "(drag jari)" : "(Mouse)"} agar matang merata.
                </p>
              </div>
            </div>
              ...
              <div className="space-y-3">
                ...
                <div className="bg-amber-900/40 border border-amber-600/30 p-3 rounded-xl">
                  <p className="text-xs text-amber-200 text-center leading-relaxed">
                    ⚠️ <strong>PENTING:</strong> Harus mengaduk sambil menuang
                    air! Jika diam = menggumpal, terlalu cepat = tumpah.
                  </p>
                </div>
              </div>
            ) : (
      ...
      {gameState === "WON" && (
        <div className="absolute inset-0 z-50 bg-emerald-950/90 flex flex-col items-center justify-center p-8 text-center backdrop-blur-md">
          ...
          <p className="text-xl text-emerald-100/80 mb-8 max-w-lg leading-relaxed">
            Tekstur bening, kenyal, dan matang merata. <br />
            Siap disajikan dengan Ikan Kuah Kuning.
          </p>
          <button
            onClick={startGame}
